...
<div x-data="{ show: true }" x-show="show" x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="rounded-md {{ $config['bg'] }} border {{ $config['border'] }} p-4 mb-4">
    <div class="flex">
        ...
        <div class="ml-auto pl-3">
            <div class="-mx-1.5 -my-1.5">
                <button @click="show = false" type="button" class="{{ $config['text'] }} hover:opacity-75 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-{{ str_replace('bg-', '', $config['bg']) }} focus:ring-{{ str_replace('border-', '', $config['border']) }} rounded-md p-1.5 inline-flex">
