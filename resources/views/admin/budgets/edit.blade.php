<x-app-layout>
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Edit Budget</h1>
                <p class="text-sm text-gray-500 mt-1">{{ $budget->category->name ?? 'N/A' }} - {{ $budget->period_name ?? $budget->year . '/' . $budget->month }}</p>
            </div>
        </div>

        <!-- Form -->
        <div class="bg-white rounded-lg border border-gray-200">
            <form action="{{ route('admin.budgets.update', $budget) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="department" id="department" class="w-full rounded-lg border-gray-300 text-sm focus:border-slate-500 focus:ring-slate-500">
                            ...
                            @foreach($departments as $dept)
                                <option value="{{ $dept }}" {{ old('department', $budget->department) == $dept ? 'selected' : '' }}>
                                    {{ $dept }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="limit_amount" id="limit_amount" value="{{ old('limit_amount', $budget->limit_amount) }}" required min="0" step="1000" 
                    ...
                    <div>
                        ...
                        <input type="number" name="warning_threshold" id="warning_threshold" value="{{ old('warning_threshold', $budget->warning_threshold) }}" required min="0" max="100" 
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="notes" id="notes" rows="3" class="w-full rounded-lg border-gray-300 text-sm focus:border-slate-500 focus:ring-slate-500" placeholder="Optional notes about this budget...">{{ old('notes', $budget->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <label class="flex items-center gap-3">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $budget->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-slate-600 focus:ring-slate-500">
                ...
                <div class="bg-slate-50 rounded-lg p-4 border border-slate-200">
                    <h4 class="text-sm font-semibold text-slate-900 mb-2">Current Usage</h4>
                    <div class="grid grid-cols-3 gap-4 text-sm">
                        <div>
                            <span class="text-gray-500">Used Amount</span>
                            <p class="font-semibold text-gray-900">Rp {{ number_format($budget->used_amount ?? 0, 0, ',', '.') }}</p>
                        </div>
                        <div>
                            <span class="text-gray-500">Remaining</span>
                            <p class="font-semibold text-emerald-600">Rp {{ number_format($budget->remaining_amount ?? $budget->limit_amount, 0, ',', '.') }}</p>
                        </div>
                        <div>
                            <span class="text-gray-500">Usage</span>
                            <p class="font-semibold {{ ($budget->usage_percentage ?? 0) > $budget->warning_threshold ? 'text-amber-600' : 'text-gray-900' }}">{{ number_format($budget->usage_percentage ?? 0, 1) }}%</p>
                        </div>
                    </div>
                </div>
